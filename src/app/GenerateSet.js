...
//     let quotes = json_string.replaceAll("'", '"')
//     let lower_caseT = quotes.replaceAll("True", "true")
//     let formatted = lower_caseT.replaceAll("False", "false")
//     return formatted
// }
...
const GenerateSet = (props) => {
  ...
  useEffect(() => {
      ...
      console.log(props.generated_set)
  ...
  if (isClient) {
    // let json_str = "{'data': {'url': 'data/day_by_day_2021.json'}, 'mark': {'type': 'bar'}, 'encoding': {'x': {'aggregate': 'count', 'field': '', 'type': ''}, 'y': {'bin': True, 'field': 'Income', 'type': 'quantitative', 'aggregate': ''}}}"
    // // json_str = '{"name":"John", "age":30, "city":"New York"}'
    // console.log(json_str.replaceAll("'", '"'))
    // console.log(JSON.parse(json_str.replaceAll("'", '"').replaceAll("True", "true")))
    // let formatted_json = parseJson(json_str)
    // embed('#pilotVis', JSON.parse(formatted_json), {"actions": false});
    // console.log(loadVis)
...
//   let transformations = tileSets[currentChartType]["transformations"];
  ...
  const displayRuleCombo = (combo) => {
    ...
    for (let entry in props.generated_set) {
      ...
      if (props.generated_set[entry]["score"] == props.score) {
        ...
        if (display) {
          ...
          for (let rule = 0; rule < props.generated_set[entry]["rules_descriptions"].length; rule += 1) {
            rules_set.add(props.generated_set[entry]["rules_descriptions"][rule])
          }
    ...
    console.log(rules_set)

    let rules_container = document.getElementById("rulesList")
    rules_container.innerHTML = ""
    // let rules_set = ""
    let passed_rules = document.createElement("div")
    passed_rules.classList.add("marginTop")
    let failed_rules = document.createElement("div")
    for (const rule of rules_set) {
      console.log(rule)
      let rules_div = document.createElement("div")
      if (rule.includes("PASSED")) {
        rules_div.innerHTML = rule
        passed_rules.appendChild(rules_div)
      } else if (rule.includes("FAILED")) {
        rules_div.innerHTML = rule
        failed_rules.appendChild(rules_div)
      }
    }
    rules_container.appendChild(failed_rules)
    rules_container.appendChild(passed_rules)
  ...
  return (
    <div id="globalContainerAnswer">
    {/* <QuestionText question={itemBank[currentItem]["question_meta_data"]}></QuestionText> */}
