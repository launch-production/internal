...
const CreateItemComponent = (props) => {
...
//   let transformations = tileSets[currentChartType]["transformations"];
    ...
    if (data.includes("data") && ev.target.getAttribute('data-draggable').includes("data_target")) {
      ...
      if (proceed) {
        ...
        //   let find_encoding = drop_container.nextSibling.firstChild.id.split("-")[1] // TODO fix; check have an unique separator
          let find_encoding = drop_container.getAttribute("data-draggable").split("-")[1]
    ...
    if (data.includes("transformation") && ev.target.getAttribute('data-draggable').includes("transformation_target")) {
      ...
      if (proceed) {
        ...
        //   let find_transformation_encoding = drop_container.previousSibling.firstChild.id.split("-")[1]
          let find_transformation_encoding = drop_container.getAttribute("data-draggable").split("-")[1]
          drop_container.firstChild.id += "-";
  ...
  return (
    ...
    {!showTextBox ? <div id='mappingZone' data-draggable="removing" onDrop={(event) => dragOff(event)} onDragOver={(event) => allowDrop(event)}>
        <p><i>Drag tiles here to remove</i></p>
    </div> : null }
            ...
            <div id='data'>
                ...
                <div id="actionTiles">
                    {Object.entries(transformations).map((transformation_tiles, index) => (
                        ...
                        <div key={"action"+index} className="transformationTileContainer">
            ...
            <div className='xContainer'>
                ...
                <div>
                    <div className='mappingContainerX'>
                        ...
                        <div className="xTiles">
                            <p>Action</p>
                            <div className='inputSpace inputTransformation' data-draggable={"transformation_target-x"} onDrop={(event) => transformationDrop(event)} onDragOver={(event) => allowDrop(event)} draggable="true" onDragStart={(event) => drag(event)}>
