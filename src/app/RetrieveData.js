...
const updateProgress = async (prolificID, completed_item) => {
  ...
  useEffect(() => {
    ...
    //   }
      ...
      var db_pid = require("./db_pid.json");
      console.log(db_pid)
  ...
  const readRecord = async (prolificID) => {
    try {
        const querySnapshot = await getDocs(collection(db, prolificID));
        var all_collections = {}
        querySnapshot.forEach((doc) => {
        // doc.data() is never undefined for query doc snapshots
        all_collections[doc.id] = doc.data();
        // if (doc.id == "progress") {
        //     progress = doc.data();
        // }
        // console.log(doc.id, " => ", doc.data());
  ...
  const readDB = async (prolific_ID) => {
    console.log("checking progress?")
    console.log(prolific_ID)
    if (prolific_ID) {
      const retrieved_data = await readRecord(prolific_ID);
      if (!retrieved_data) {
        // setPID("");
        // setScore("");
        alert("An error occurred. Please contact the survey administrator.");
    ...
    const read_all = async (db_pid) => {
        let all_pid = {}
        for (let i = 0; i < db_pid.length; i += 1) {
            const one_participant_data = await readDB(db_pid[i]["PID"])
            if (!one_participant_data) {
                // setPID("");
                // setScore("");
                alert("An error occurred. Please contact the survey administrator.");
                return false
              } else {
                console.log(one_participant_data)
                all_pid[db_pid[i]["PID"]] = one_participant_data
                // return retrieved_data
              }}
    
        console.log(all_pid)

        let blob = new Blob([JSON.stringify(all_pid)], { type: 'application/json' })
